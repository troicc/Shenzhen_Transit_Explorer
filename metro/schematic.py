...
本文件中的纯数学函数刻意与 ``static/learn/geometry.js`` + ``static/learn/core.js``
...
# ---------------------------------------------------------------------------
# 区间/折线工具 — 对应 static/learn/geometry.js 顶部
# ---------------------------------------------------------------------------
...
def _default_normalize(value: str) -> str:
    """惰性导入 matcher.normalize_name（避免与 build_network 循环导入）。"""
