...
function hashColor(text){let h=0;for(const ch of String(text))h=(h*31+ch.charCodeAt(0))%360;return `hsl(${h} 78% 61%)`}
function intersects(a,b){return a[2]>=b.x&&a[0]<=b.x+b.w&&a[3]>=b.y&&a[1]<=b.y+b.h}
...
function fitHome(){const world=state.overview.world;fitRect([0,0,world.width,world.height],.035);state.homeView={...state.view}}
...
function render(){
  ...
  for(const route of routes){
    ...
    routeCtx.strokeStyle=meta.match_state==='review'?'rgba(255,208,111,.16)':'rgba(122,165,218,.16)';
    routeCtx.lineWidth=zoom<2?0.75:1.05;drawPath(routeCtx,route.path,t);
  }
  if(state.selected){
    const route=state.selected, path=route.paths?.detail||route.path;
    routeCtx.strokeStyle='rgba(3,8,16,.92)';routeCtx.lineWidth=9;drawPath(routeCtx,path,t);
    routeCtx.strokeStyle=hashColor(route.route_no);routeCtx.lineWidth=5.2;drawPath(routeCtx,path,t);
  }
  if(zoom>=2.6){
    for(const st of state.viewData.stations||[]){
      ...
      labelCtx.beginPath();labelCtx.arc(x,y,st.route_count>=4?3.1:2.0,0,Math.PI*2);labelCtx.fillStyle=st.estimated?'rgba(255,208,111,.72)':'rgba(238,246,255,.72)';labelCtx.fill();
      if(zoom>=8&&st.route_count>=2){drawLabel(labelCtx,st.name,x+5,y-4,10)}
    }
  }
  if(state.selected){
    const stops=state.selected.stops||[];
    for(const [index,st] of stops.entries()){
      const [x,y]=worldToScreen(st.x,st.y,t);labelCtx.beginPath();labelCtx.arc(x,y,index===0||index===stops.length-1?4.5:3.2,0,Math.PI*2);labelCtx.fillStyle=st.estimated?'#ffd06f':'#fff';labelCtx.fill();labelCtx.strokeStyle='#07101d';labelCtx.lineWidth=1.7;labelCtx.stroke();
      if(zoom>=3.4||index===0||index===stops.length-1)drawLabel(labelCtx,st.name,x+6,y-5,11,st.estimated?'#ffd98b':'#eaf2fc')
    }
  }
}
...
function scheduleViewFetch(){clearTimeout(state.fetchTimer);state.fetchTimer=setTimeout(fetchView,120)}
async function fetchView(){
  ...
  const v=state.view,z=currentZoom(),url=`/api/network/view?minx=${v.x}&miny=${v.y}&maxx=${v.x+v.w}&maxy=${v.y+v.h}&zoom=${z}`;
  try{const r=await fetch(url,{signal:state.fetchController.signal});if(!r.ok)throw new Error();state.viewData=await r.json();render()}catch(error){if(error.name!=='AbortError')console.warn('view fetch failed')}
}
...
function hitRoute(screenX,screenY){
  const t=transform();let best=null,bestD=9;for(const route of routeSource()){
...
async function selectRoute(id,fit=true){
  try{const route=await api(`/api/network/route/${encodeURIComponent(id)}`);state.selected=route;renderRoutePanel(route);if(fit)fitRect(route.bbox,.18);else render();showToast(`${route.route_no} · ${route.direction_label}`)}catch(error){showToast(error.message)}
}
function renderRoutePanel(route){
  els.routeCard.innerHTML=`<h3>${escapeHtml(route.route_no)} · ${escapeHtml(route.direction_label)}</h3><p>${escapeHtml(route.start_stop)} → ${escapeHtml(route.end_stop)}</p><p>${escapeHtml(route.operator)}</p><div><span class="badge ${escapeHtml(route.match_state)}">${escapeHtml(route.match_state)}</span> <span class="badge">匹配 ${Math.round(route.score*100)}%</span> <span class="badge">${route.stops.length} 站</span></div>`;
  ...
  els.stopList.querySelectorAll('[data-index]').forEach(node=>node.addEventListener('click',()=>{const st=route.stops[Number(node.dataset.index)];const width=state.homeView.w/12;fitRect([st.x-width/2,st.y-width/2,st.x+width/2,st.y+width/2],0);render()}));
}
...
function renderSearch(results){
  ...
  els.searchResults.querySelectorAll('[data-result]').forEach(node=>node.addEventListener('click',()=>{const r=results[Number(node.dataset.result)];els.searchResults.innerHTML='';if(r.type==='route')selectRoute(r.id,true);else{const width=state.homeView.w/16;fitRect([r.x-width/2,r.y-width/2,r.x+width/2,r.y+width/2],0);showToast(r.title)}}));
}
els.search.addEventListener('input',()=>{clearTimeout(state.searchTimer);const q=els.search.value.trim();if(!q){els.searchResults.innerHTML='';return}state.searchTimer=setTimeout(async()=>{try{const data=await api(`/api/search?q=${encodeURIComponent(q)}&limit=40`);renderSearch(data.results||[])}catch{}},180)});
...
(async function init(){
  try{
    const overview=await api('/api/network/overview');state.overview=overview;state.routeMap=new Map(overview.routes.map(r=>[r.id,r]));
    els.networkMeta.textContent=`${overview.stats.directions} 个方向 · ${overview.stats.station_clusters} 个站点簇 · 构建于 ${new Date(overview.built_at).toLocaleString()}`;
    els.topStatus.innerHTML=`方向线路 <b>${overview.stats.directions}</b> · 站点簇 <b>${overview.stats.station_clusters}</b>`;
    resize();state.viewData={routes:overview.routes,stations:[]};fitHome();els.loading.classList.add('hidden');scheduleViewFetch();
  }catch(error){
    els.loadingTitle.textContent='尚未生成全网数据';els.loadingText.innerHTML=`${escapeHtml(error.message)}<br><br><a class="button primary" href="/bus/collector">进入数据采集</a>`;els.networkMeta.textContent='需要先采集并构建。';
  }
})();
