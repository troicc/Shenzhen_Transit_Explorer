...
// 复用 static/learn/core.js 与 geometry.js 的几何原语，保证与 Python 端同算法。
import { clamp, svgEl, distance, pointAtProgress, pathMetrics, pathD, showToast } from '/static/learn/core.js';
import { projectPointToPath } from '/static/learn/geometry.js';
...
$('#saveBtn').addEventListener('click', async () => {
  ...
  try {
    const res = await fetch('/api/metro/schematic/save', {
      method: 'POST',
...
async function load() {
  try {
    const res = await fetch('/api/metro/schematic');
    const data = await res.json();
