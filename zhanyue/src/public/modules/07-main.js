(() => {
  ...
  const elements = Object.fromEntries([
    'app','overview','focus','hero','routeMap','routeDock','routeCode','routeName','routeMeta','lineStats','lineStrip',
  ...
  function renderManifest(manifest) {
    ...
    manifest.lines.forEach(line => {
      ...
      button.textContent = line.id === '2' ? '2/8' : line.id.replace('号线', '');
  ...
  function updateLineAndStation() {
    ...
    elements.routeCode.textContent = state.line.id === '2' ? '2/8' : state.line.id;
  ...
  function startPractice(mode = 'full') {
    ...
    elements.feedback.textContent = '输入正确后，列车前往下一站。';
