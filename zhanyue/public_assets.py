"""Build and audit geometry-safe public derivatives.
...
def _build_base_rasters(
    points: Sequence[Point], stations: Sequence[Mapping[str, Any]], color: Tuple[int, int, int], output: Path
) -> List[Dict[str, Any]]:
    from PIL import Image, ImageDraw

    metrics = path_metrics(points)
    levels: List[Dict[str, Any]] = []
    for scale in RASTER_LEVELS:
        ...
        levels.append({"scale": scale, "url": f"/assets/routes/{output.name}/{filename}"})
    return levels
...
def _build_segment_assets(
    points: Sequence[Point], start: float, end: float, index: int, color: Tuple[int, int, int], output: Path
) -> Dict[str, Any]:
    from PIL import Image, ImageDraw
    ...
    for direction in ("forward", "reverse"):
        ...
        atlas_urls[direction] = f"/assets/routes/{output.name}/{atlas_name}"
    ...
    return {
        ...
        "origin": [left, top],
        "size": [width, height],
        "completedUrl": f"/assets/routes/{output.name}/{completed_name}",
...
def build_route_scene(line: Mapping[str, Any], output_root: Path) -> Tuple[Dict[str, Any], Dict[str, int]]:
    line_id = str(line.get("id", ""))
    safe_id = safe_line_id(line_id)
    output = output_root / "assets" / "routes" / safe_id
    points = fit_path(parse_path(str(line.get("d", ""))))
    stations = list(line.get("stations", []))
    metrics = path_metrics(points)
    color = _rgb(str(line.get("color", "#5cc8ff")))
    levels = _build_base_rasters(points, stations, color, output)
    public_stations = []
    for station in stations:
    ...
    return scene, {"rasters": len(levels), "segments": len(segments), "atlases": len(segments) * 2}
...
def build_public_derivatives(
    data: Mapping[str, Any], output: Path, build_id: str, geographic_path: Optional[Path] = None
) -> Dict[str, Any]:
    ...
    for line in data.get("lines", []):
        line_id = str(line.get("id", ""))
        safe_id = safe_line_id(line_id)
        scene, counts = build_route_scene(line, output)
        for key, value in counts.items():
            counters[key] += value
        line_dir = output / "lines" / safe_id
        write_json(line_dir / "scene.json", scene)
        geo_payload = build_geographic_payload(line, geographic[line_id]) if line_id in geographic else None
        modes = ["flat"]
        if geo_payload:
    ...
    write_json(output / "search-index.json", {"stations": list(search_records.values())})
