...
const {clamp, lerp, stationTravelPhase, stationVisualState} = Z;
...
class RealMapFocusRenderer {
  ...
  async ensureMap() {
    if (this.map) return;
    const response = await fetch(`/api/${window.TransitPublic.api.network}/runtime`);
    if (!response.ok) throw new Error('无法读取真实地图配置');
    const config = (await response.json()).amap || {};
...
Z.RealMapFocusRenderer = RealMapFocusRenderer;
Z.geoMetrics = geoMetrics;
Z.geoPathSlice = geoPathSlice;
Z.geoPointAtProgress = geoPointAtProgress;
Z.normalizeRotation = normalizeRotation;
Z.shortestRotationDelta = shortestRotationDelta;
Z.smoothedGeoDirection = smoothedGeoDirection;
})();
