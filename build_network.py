...
from db import DATA_DIR, DB_PATH, iter_direction_matches, normalize_stop, text_similarity
...
def publish_split_data(network: Dict[str, Any], data_dir: Path = DATA_DIR) -> Dict[str, Any]:
    ...
    manifest = {
        "version": network["version"],
        "built_at": network["built_at"],
        "overview_path": "bus/overview.json.gz",
...
def build(db_path: Path = DB_PATH, output_path: Optional[Path] = None, include_review: bool = True) -> Dict[str, Any]:
    output_path = output_path or (DATA_DIR / "network.json.gz")
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=DATA_DIR / "network.json.gz")
